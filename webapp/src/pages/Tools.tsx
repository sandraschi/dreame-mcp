...
import { Wrench, Radio, Copy, Check } from 'lucide-react'
    ...
    <div className="space-y-6 py-4 max-w-4xl">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm">Dreame D20 Pro tools for AI clients (Cursor, Claude Desktop)</p>
        </div>
      </div>
      <div className="rounded-2xl border border-white/10 bg-[#0f0f12]/80 p-5 space-y-4">
        <p className="text-sm text-slate-400">
          Connect your MCP client to <code className="text-amber-400">http://localhost:10794/sse</code> (SSE transport).
        </p>
        <div>
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Portmanteau</p>
          <p className="text-sm text-slate-400">dreame(operation=&quot;status&quot;), dreame(operation=&quot;map&quot;), dreame(operation=&quot;start_clean&quot;), dreame(operation=&quot;stop&quot;), dreame(operation=&quot;go_home&quot;), dreame(operation=&quot;find_robot&quot;), dreame(operation=&quot;battery&quot;)</p>
        </div>
        <div>
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Help</p>
          <p className="text-sm text-slate-400">dreame_help(category=...) — categories: status, map, control, connection</p>
        </div>
        <div>
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Agentic workflow (SEP-1577)</p>
          <p className="text-sm text-slate-400">dreame_agentic_workflow(goal=&quot;...&quot;) — LLM plans and runs get_status, get_map, start_clean, stop, go_home, find_robot</p>
        </div>
        <div>
