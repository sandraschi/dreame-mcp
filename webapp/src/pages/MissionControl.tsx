...
export default function MissionControl() {
	...
	const [isNavOpen, _setIsNavOpen] = useState(true);
		...
		<div className="flex h-screen bg-[#05070a] text-slate-100 overflow-hidden">
			...
			>
				<div className="p-6 flex items-center gap-4">
					...
					{isNavOpen && (
						<span className="font-bold tracking-tight text-lg">DREAME-MCP</span>
					)}
				</div>
			...
			<main className="flex-1 overflow-y-auto relative p-8">
				...
				<header className="flex items-center justify-between mb-8">
					...
					<div className="flex gap-4">
						<StatCard
							icon={<Battery className="text-emerald-400" size={16} />}
							label="Battery"
							value={`${telemetry?.battery}%`}
						/>
						<StatCard
							icon={<Clock className="text-indigo-400" size={16} />}
							label="Runtime"
							value={`${Math.floor((telemetry?.cleaning_time || 0) / 60)}m`}
						/>
						<StatCard
							icon={<AreaChart className="text-sky-400" size={16} />}
							label="Area"
							value={`${telemetry?.cleaned_area}m²`}
						/>
					</div>
				</header>
				...
				<div className="grid grid-cols-12 gap-6">
					{/* Map View */}
					<section
						className="col-span-12 xl:col-span-8 h-[600px] animate-fade-in opacity-0"
					...
					<section className="col-span-12 xl:col-span-4 space-y-6">
						{/* Command Center */}
						<div
							className="glass-card rounded-2xl p-6 animate-fade-in opacity-0"
...
function NavButton({ icon, label, active = false, collapsed = false }: any) {
	return (
		<button
			type="button"
			className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all ${active ? "bg-indigo-600/10 text-indigo-400" : "text-slate-400 hover:bg-white/5 hover:text-slate-200"}`}
		>
			{icon}
...
function ControlButton({ icon, label, variant }: any) {
	const styles: any = {
		...
		outline:
			"bg-transparent border border-white/10 hover:bg-white/5 text-slate-400 hover:text-slate-200",
	};
	return (
		<button
			type="button"
			className={`flex flex-col items-center justify-center py-4 rounded-xl transition-all gap-2 ${styles[variant]}`}
		>
			{icon}
	...
	return (
		<div className="space-y-1.5">
			<div className="flex justify-between text-[10px] font-bold">
				...
				<span className={warning ? "text-amber-400" : "text-indigo-400"}>
					{percent}%
				</span>
			</div>
