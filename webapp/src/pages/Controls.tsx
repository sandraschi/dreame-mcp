...
import { PlayCircle, Square, Home, MapPin, Loader2, AlertCircle } from 'lucide-react'
  ...
  return (
    <div className="space-y-6 py-4 max-w-4xl">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Controls</h1>
          <p className="text-slate-400 text-sm">Start, stop, pause, return to dock. Requires DREAME_IP and DREAME_TOKEN (miio).</p>
        </div>
      </div>
      ...
      {msg && (
        <div
          className={`flex items-center gap-3 p-4 rounded-2xl border ${
            msg.type === 'ok' ? 'border-green-500/20 bg-green-500/10 text-green-200' : 'border-amber-500/20 bg-amber-500/10 text-amber-200'
          }`}
        >
